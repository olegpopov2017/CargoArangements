...
import {
  create_cuboid_from_cargo_area,
  create_cargo_from_cuboid,
  threejs_scena_to_cuboid_with_inner_objects,
  create_cuboid_from_cargo,
  cargo_area_adding_from_cuboid,
} from './functions'
...
export function placement_cargo_according_to_algorithm_on_internet_server() {
  let box = threejs_scena_to_cuboid_with_inner_objects()
  console.log(box)

  fetch('http://185.253.182.115/api/algotitm', {
    method: 'post',
...
export function present_object_parameters() {
  try {
    ...
    if (typeof car_area === 'undefined') {
      throw new Error(
        'Cargo area not createted. Please create cargo area and try again.'
      )
    }
    ...
    if (
      ...
      throw new Error(
        'Cargo area size not valid. Please enter correct size and try again.'
      )
    }

    if (cargo_group.children.length == 0) {
      throw new Error('No objects in cargo area.')
    }
    ...
    console.log(
      ...
      car_area.depth_Z,
      '\n',
      'Number of objects in the cargo area:',
      cargo_group.children.length,
      '\n'
    )
    ...
    for (let i = 0; i < cargo_group.children.length; ++i) {
      ...
      if (cargo.width_X == 0 || cargo.height_Y == 0 || cargo.depth_Z == 0) {
        console.log(
          'Cargo uuid',
          cargo.uuid,
          'size not valid. Please enter correct size and try again.'
        )
      }
